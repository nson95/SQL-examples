...
SELECT VendorID, InvoiceDate, InvoiceTotal,
	SUM(InvoiceTotal) OVER(PARTITION BY VendorID) AS InvoiceSum,
	COUNT(InvoiceTotal) OVER(PARTITION BY VendorID) AS InvoiceCount,
	AVG(InvoiceTotal) OVER(PARTITION BY VendorID) AS InvoiceAvg
FROM Invoices
